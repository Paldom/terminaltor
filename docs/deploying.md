...
- Consumers run `npx skills add Paldom/terminaltor` — the CLI
  ...
  users can also `gh skill install Paldom/terminaltor <skill>
...
gh repo edit Paldom/terminaltor --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/terminaltor/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/terminaltor --add-topic skills-sh

# 4. Verify installability exactly like a consumer
npx skills add Paldom/terminaltor --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
npx skills add Paldom/terminaltor --skill '*' -a claude-code -y
...
(`https://skills.sh/b/Paldom/terminaltor`) linking to the repo's
skills.sh page.
